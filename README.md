...
- **Agor Background & Prompt Integration** — includes `agor-bg` and `agor-prompt` CLI tools and skills for handling long-running tasks.
...
## Installation (Agor CLI & Skills)

To use the background task and prompt capabilities:

1. **Install CLI Tools**:
   ```bash
   cp <repo_path>/cli/agor-bg ~/.local/bin/
   cp <repo_path>/cli/agor-prompt ~/.local/bin/
   chmod +x ~/.local/bin/agor-bg ~/.local/bin/agor-prompt
   ```

2. **Install Skill**:
   ```bash
   cp -r <repo_path>/skills/agor-bg ~/.claude/skills/
   ```

3. **Configure Environment**:
   Set the following environment variables in your `.bashrc` or `.zshrc`:
   ```bash
   export AGOR_DAEMON_URL="http://localhost:3031"
   export AGOR_TOKEN="your_actual_jwt_token_here"
   ```
